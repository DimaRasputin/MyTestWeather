...
public class Optimum implements Navigable {

    private static final Logger logger = LogManager.getLogger(Optimum.class.getSimpleName());
    ...
    public void navigate(DriverWrapper driverWrapper, Context context, Reporter reporter) {
        ...
        new Click(ByHandler.getBy(context, "novest.testNovest.userInterface.optimum"))
                .act(driverWrapper, context, reporter);
        new WaitFor(ByHandler.getBy(context, "common.flagPageLoad"),
                Const.PAGE_COMPLETELY_LOADED_TIMEOUT).act(driverWrapper, context, reporter);
    }

    public Optimum clickRequestACall (DriverWrapper driverWrapper, Context context, Reporter reporter) {
