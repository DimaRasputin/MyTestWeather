...
public class Enterprise implements Navigable {

    private static final Logger logger = LogManager.getLogger(Enterprise.class.getSimpleName());
    ...
    public void navigate(DriverWrapper driverWrapper, Context context, Reporter reporter) {
        ...
        new Click(ByHandler.getBy(context, "novest.testNovest.userInterface.enterprise"))
                .act(driverWrapper, context, reporter);
        new WaitFor(ByHandler.getBy(context, "common.flagPageLoad"),
                Const.PAGE_COMPLETELY_LOADED_TIMEOUT).act(driverWrapper, context, reporter);
    }

    public Enterprise clickRequestACall (DriverWrapper driverWrapper, Context context, Reporter reporter) {
